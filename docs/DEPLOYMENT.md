...
- Environment:
  ...
  - `PERSISTENCE_DRIVER=prisma` after the API image `release_command` has
    applied migrations (and seed). Named `live`
  ...
  - Optional authenticator MFA: after the first email/dev MFA verification,
    ...
    `POST /v1/auth/mfa/recovery-codes/regenerate` replaces the set. Optional
    hosted identity overlay: `AUTH_OIDC_ISSUER` and `AUTH_OIDC_AUDIENCE` enable
