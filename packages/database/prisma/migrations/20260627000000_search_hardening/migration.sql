...
UPDATE "AdvertDiscoveryIndex" SET "searchVector" = to_tsvector("english", coalesce("title","") || " " || coalesce("displayName","") || " " || coalesce("description","") || " " || coalesce("industryCode","") || " " || coalesce("role",""));

CREATE INDEX IF NOT EXISTS "AdvertDiscoveryIndex_searchVector_idx" ON "AdvertDiscoveryIndex" USING GIN("searchVector");
CREATE INDEX IF NOT EXISTS "AdvertDiscoveryIndex_embedding_idx" ON "AdvertDiscoveryIndex" USING ivfflat("embedding" vector_cosine_ops) WITH (lists = 100);
...
CREATE OR REPLACE FUNCTION update_advert_search_vector() RETURNS TRIGGER AS $$ BEGIN NEW."searchVector" := to_tsvector("english", coalesce(NEW."title","") || " " || coalesce(NEW."displayName","") || " " || coalesce(NEW."description","") || " " || coalesce(NEW."industryCode","") || " " || coalesce(NEW."role","")); RETURN NEW; END; $$ LANGUAGE plpgsql;
